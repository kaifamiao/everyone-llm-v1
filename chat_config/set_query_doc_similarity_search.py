...
from chat_config.CustomDirectoryLoader import CustomDirectoryLoader
from chat_config.CustomDocumentLoader import CustomDocumentLoader
...
def get_session_history(session_id: str) -> BaseChatMessageHistory:
    kfm_logger.warning(f"get_session_history session_id: {session_id}")
    if session_id=="":
...
def kfm_query_doc_with_similarity_search(user_input="",config={}):
    ...
    if st.session_state.DOC_DIALOG_FILENAME is not None and len(st.session_state.DOC_DIALOG_FILENAME) >0:
        ...
        full_file_name = file_path + "/" + st.session_state.DOC_DIALOG_FILENAME[0]
        kfm_logger.debug(f"📄💬 full_file_name \n\t {full_file_name}")
        print("full_file_name : \n")
        print(full_file_name)

        kfm_logger.warning(f"Enter is st.session_state.DOC_DIALOG_FILENAME {st.session_state.DOC_DIALOG_FILENAME} ✅🔍🔍🔍🔍🔍🔍 ")

        # loader = DirectoryLoader(file_path, glob=st.session_state.DOC_DIALOG_FILENAME, show_progress=True, use_multithreading=True)
        loader = CustomDirectoryLoader(file_path, glob=st.session_state.DOC_DIALOG_FILENAME, show_progress=True, use_multithreading=True)
        # loader = CustomDocumentLoader(full_file_name)
        ...
        collection_name = string_to_md5(config.get('configurable').get('session_id'))
        print(collection_name)
        kfm_logger.debug(f"Generate collection_name : {collection_name}")
        ...
        k=3
        print(f"similar_docs = vectorstore.similarity_search({user_input}, k={k})")
        similar_docs = vectorstore.similarity_search(user_input, k=k)

        kfm_logger.debug(f"similar_docs : {similar_docs}")
        ...
        kfm_logger.debug(f"Create prompt by ChatPromptTemplate.from_messages")
        ...
        kfm_logger.debug(f"Create chain by RunnablePassthrough| trimmer) | promptx | kfm_llm")
        ...
        kfm_logger.debug(
            f"🤖 Create with_message_history by  {st.session_state.DOC_DIALOG_FILENAME} ,t.session_state.session_id ： {st.session_state.session_id}\n\n")
        config = {"configurable": {"session_id": st.session_state.session_id}}
        kfm_logger.debug(config)


        response_stream2 = ai_with_message_history.stream(
            {"input": [HumanMessage(content=user_input)]},
            config=config)

        print("######################################### RunnableWithMessageHistory ################################################################")
        print("get_session_history success! ")
        print("get_session_history success! ")
        # for chunk in response_stream2:
        #     print(chunk.content, end="", flush=True)

        # kfm_logger.debug(f"response_stream : {response_stream2}")
        return response_stream2
